...
### Layer 1: ###
	...
	#Whenever I want to use the Relu activation function to check its effects on the accuracy 
	...
	#However, by using ReLU, the performance is very poor. It is better to use
	#A sigmoid activation function for this particular case. 
	...
	#Uncomment the following line if you want to use cross entropy as the cost function
	# cross_entropy = tf.reduce_mean(-tf.reduce_sum( y_onehot * tf.log(aL+eps) +  (1.0-y_onehot )*tf.log(1.0-aL +eps) , reduction_indices=[1]))

	#and now let's try another cost function in order to see how the ouput changes with the choice of the cost-function
	MSE = (1/N_train)*tf.reduce_sum(tf.square(aL - y_onehot))
	...
	filename = "Data/n_%i"%nH
	outsample = open(filename, mode="w")
	outsample.write("#epoch accuracy cost\n")
	...
	for i in range(len(epoch_list)):
		outsample.write("{} {} {}\n".format(epoch_list[i], acc_training[i], cost_training[i]))
	outsample.close()

	# plt.savefig('spiral_results.png') # Save the figure showing the results in the current directory
