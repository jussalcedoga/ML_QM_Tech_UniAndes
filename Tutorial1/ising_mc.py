...
n_measSweeps = 1000  #number of measurement sweeps

### Parameters needed to show animation of spin configurations: ###
animate = False
bw_cmap = colors.ListedColormap(['blue', 'red'])
...
def getEnergy():
  ...
  for i in range(N_spins):
    currEnergy += -J*( spins[i]*spins[neighbours[i,0]] + spins[i]*spins[neighbours[i,1]] + \
                    spins[i]*spins[neighbours[i,2]] + spins[i]*spins[neighbours[i,3]])
  return 0.5*currEnergy
#end of getEnergy() function

def localEnergy(site):
    energy = 0
    for i in range(4):
        energy += -J * spins[site] * spins[neighbours[site, i]]
    return energy
...
def sweep():
  ...
  for i in range(N_spins):
    ...
    # *********************************************************************** #
    # deltaE = 2 * J * spins[site] * np.sum(spins[neighbours[site]])
